...
export default function VigoDrops() {
  ...
  useEffect(() => {
    base44.entities.LiveStreamEvent.filter({ isLive: true }, "-created_date", 1)
      .then(rows => setLiveStream(rows?.[0] || null))
      .catch(() => {});
  }, []);
  ...
  return (
    <PullToRefresh onRefresh={handleRefresh}>
      <div>
        {/* Live Now Banner */}
        {liveStream && (
          <Link to="/live" style={{ display: "block", textDecoration: "none", background: "#0a0a0a", borderBottom: ".5px solid #1a1a1a", padding: "14px 20px" }}>
            <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,60,60,0.12)", border: ".5px solid rgba(255,60,60,0.3)", borderRadius: 6, padding: "5px 10px" }}>
                  <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#ff3b3b", animation: "vigo-pulse 1.5s infinite" }} />
                  <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: 2, color: "#ff5252", textTransform: "uppercase" }}>Live Now</span>
                </div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 900, color: "#fff", letterSpacing: 1 }}>{liveStream.title || "THE BACK DOOR"}</div>
                  {liveStream.viewerCount > 0 && <div style={{ fontSize: 9, color: "rgba(255,255,255,0.45)", marginTop: 1 }}>{liveStream.viewerCount.toLocaleString()} watching</div>}
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#ff3b3b", borderRadius: 6, padding: "8px 16px", flexShrink: 0 }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: 1, color: "#fff", textTransform: "uppercase" }}>Watch</span>
              </div>
            </div>
          </Link>
        )}
            ...
            <div style={{ background: G1, border: `.5px solid ${G3}`, borderTop: `2px solid ${S}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
                {cells.map((day, i) => {
                  ...
                  const isLiveDay = drop && liveStream && liveStream.associatedDropId === drop.id;
                  return (
                    <div
                      key={i}
                      onClick={() => {
                        if (isLiveDay) { navigate("/live"); return; }
                        if (drop) { setSelectedDrop(prev => prev?.id === drop.id ? null : drop); setViewDate(new Date(drop.date.getFullYear(), drop.date.getMonth(), 1)); }
                      }}
                    ...
                    >
                      {day && (
                        <>
                          ...
                          {drop && (
                            <div style={{ background: `${drop.tagColor}18`, borderLeft: `2px solid ${isLiveDay ? "#ff3b3b" : drop.tagColor}`, padding: "2px 4px", fontSize: 6, letterSpacing: .5, color: isLiveDay ? "#ff5252" : drop.tagColor, lineHeight: 1.4, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                              {isLiveDay ? "● LIVE" : drop.series}
                            </div>
                          )}
                        </>
                      )}
              ...
              <div style={{ padding: "12px 20px", borderTop: `.5px solid ${G3}`, display: "flex", gap: 16, flexWrap: "wrap" }}>
                {[[S, "Upcoming"], ["#888", "Coming Soon"], ["#e03", "Sold Out"], ["#ff3b3b", "Live"]].map(([c, l]) => (
