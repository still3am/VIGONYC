...
export default function VigoCartDrawer({ open, onClose }) {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, zIndex: 400, width: "min(100vw, 420px)", background: G1, borderLeft: `.5px solid ${G3}`, display: "flex", flexDirection: "column", transform: open ? "translateX(0)" : "translateX(100%)", transition: "transform .3s cubic-bezier(.4,0,.2,1)", boxShadow: open ? "-20px 0 40px rgba(0,0,0,.5)" : "none" }}>
        {/* Header */}
        <div style={{ padding: "clamp(16px,3vw,24px) clamp(16px,4vw,24px)", borderBottom: `.5px solid ${G3}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, background: `linear-gradient(180deg, ${G1}, ${G2})` }}>
          <div>
            <div style={{ fontSize: 10, letterSpacing: 3, textTransform: "uppercase", color: "#fff", fontWeight: 700, marginBottom: 2 }}>Your Bag</div>
            <div style={{ fontSize: 9, color: SD, letterSpacing: 1 }}>{items.length} {items.length === 1 ? "item" : "items"}</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: SD, cursor: "pointer", padding: 4, display: "flex", alignItems: "center", justifyContent: "center", transition: "color .2s" }} onMouseEnter={e => e.target.style.color = "#fff"} onMouseLeave={e => e.target.style.color = SD}>
            <X size={18} />
          </button>
        </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, border: `.5px solid ${G3}` }}>
                    <button onClick={() => updateQty(item.id, -1)} style={{ background: "none", border: "none", color: SD, cursor: "pointer", width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit", fontSize: 14, transition: "color .2s" }} onMouseEnter={e => e.target.style.color = "#fff"} onMouseLeave={e => e.target.style.color = SD}>−</button>
                    <div style={{ fontSize: 10, color: "#fff", width: 24, textAlign: "center", borderLeft: `.5px solid ${G3}`, borderRight: `.5px solid ${G3}` }}>{item.qty}</div>
                    <button onClick={() => updateQty(item.id, 1)} style={{ background: "none", border: "none", color: SD, cursor: "pointer", width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit", fontSize: 14, transition: "color .2s" }} onMouseEnter={e => e.target.style.color = "#fff"} onMouseLeave={e => e.target.style.color = SD}>+</button>
                  </div>
                </div>
              </div>
        ...
        <div style={{ padding: "clamp(16px,3vw,24px) clamp(16px,4vw,24px)", borderTop: `.5px solid ${G3}`, display: "flex", flexDirection: "column", gap: 12, flexShrink: 0, background: `linear-gradient(180deg, ${G2}, ${G1})` }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 900, color: "#fff" }}>${subtotal.toFixed(2)}</span>
            </div>
