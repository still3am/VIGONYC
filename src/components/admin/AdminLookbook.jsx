...
function EntryModal({ entry, onSave, onClose }) {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 600, background: "#0f0f0f", border: `0.5px solid ${G3}`, borderTop: `2px solid ${S}`, width: "min(560px,95vw)", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ padding: "20px 24px", borderBottom: `0.5px solid ${G3}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 11, fontWeight: 900, letterSpacing: 2, color: "#fff", textTransform: "uppercase" }}>{entry?.id ? "Edit Entry" : "New Lookbook Entry"}</div>
        ...
        <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 8, letterSpacing: 2, color: SD, textTransform: "uppercase", marginBottom: 8 }}>Image</div>
            {form.image_url && (
              <img src={form.image_url} alt="preview" style={{ width: "100%", maxHeight: 180, objectFit: "cover", marginBottom: 10, border: `0.5px solid ${G3}` }} />
            )}
            <label style={{ display: "block", background: G2, border: `0.5px solid ${G3}`, padding: "12px 16px", cursor: "pointer", fontSize: 11, color: SD, textAlign: "center" }}>
              {uploading ? "Uploading..." : "Click to upload image"}
              <input type="file" accept="image/*" onChange={handleUpload} style={{ display: "none" }} />
            </label>
            <div style={{ fontSize: 9, color: SD, marginTop: 6 }}>or paste URL below</div>
            <input value={form.image_url ?? ""} onChange={e => set("image_url", e.target.value)} placeholder="https://..." style={{ width: "100%", background: G2, border: `0.5px solid ${G3}`, color: "#fff", padding: "10px 14px", fontSize: 11, outline: "none", boxSizing: "border-box", fontFamily: "inherit", marginTop: 6 }} />
          </div>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
            ...
            <span style={{ fontSize: 11, color: "#fff" }}>Featured (hero entry)</span>
          </label>

          <div style={{ display: "flex", gap: 10, paddingTop: 8 }}>
            <button onClick={handleSave} disabled={saving || uploading} style={{ background: S, color: "#000", border: "none", padding: "12px 28px", fontSize: 9, letterSpacing: 3, textTransform: "uppercase", fontWeight: 900, cursor: "pointer", fontFamily: "inherit", opacity: saving ? 0.7 : 1 }}>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }} className="admin-3col">
        {!loading && entries.map(e => (
          <div key={e.id} style={{ background: G1, border: `0.5px solid ${G3}`, overflow: "hidden" }}>
            ...
            <div style={{ padding: "14px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#fff", marginBottom: 4 }}>{e.title}</div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={() => setModal(e)} style={{ flex: 1, background: "none", border: `0.5px solid ${G3}`, color: SD, padding: "7px", fontSize: 9, cursor: "pointer", fontFamily: "inherit", letterSpacing: 1, textTransform: "uppercase" }}>Edit</button>
                <button onClick={() => handleDelete(e.id)} style={{ background: "none", border: `0.5px solid #e03`, color: "#e03", padding: "7px 12px", fontSize: 9, cursor: "pointer", fontFamily: "inherit" }}>Del</button>
              </div>
            </div>
          </div>
        ))}
