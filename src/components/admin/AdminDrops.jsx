    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 600, background: "#0f0f0f", border: `0.5px solid ${G3}`, borderTop: `2px solid ${S}`, width: "min(580px,96vw)", maxHeight: "92vh", overflowY: "auto" }}>
        <div style={{ padding: "16px 20px", borderBottom: `0.5px solid ${G3}`, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "#0f0f0f", zIndex: 1 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", color: SD, fontSize: 20, cursor: "pointer" }}>✕</button>
        </div>
        <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            {form.image && (
              <div style={{ position: "relative", width: "100%", height: 140, marginBottom: 8, overflow: "hidden", background: G2 }}>
            ...
            <label style={{ display: "inline-flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 6 }}>
              <span style={{ background: G2, border: `0.5px solid ${G3}`, color: SD, padding: "8px 16px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase" }}>
          ...
          <div>
            ...
            <div style={{ maxHeight: 160, overflowY: "auto", border: `0.5px solid ${G3}`, background: G2 }}>
              {allProducts.map(p => (
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <h2 style={{ fontSize: "clamp(20px,5vw,28px)", fontWeight: 900, letterSpacing: -1, color: "#fff" }}>Drops</h2>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <div style={{ display: "flex", border: `0.5px solid ${G3}` }}>
            {[["list","List"],["calendar","Cal"]].map(([v,l]) => (
              <button key={v} onClick={() => setView(v)} style={{ background: view === v ? S : G2, color: view === v ? "#000" : SD, border: "none", borderRight: `0.5px solid ${G3}`, padding: "10px 16px", fontSize: 8, letterSpacing: 2, textTransform: "uppercase", fontWeight: view === v ? 900 : 400, cursor: "pointer", fontFamily: "inherit" }}>{l}</button>
            ))}
          </div>
      ...
      {view === "list" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {!loading && drops.map(d => (
            <div key={d.id} style={{ background: G1, border: `0.5px solid ${G3}`, borderLeft: `3px solid ${statusColor(d.status)}`, padding: "16px 20px" }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap", alignItems: "center" }}>
                  {["upcoming","live","soldout"].map(s => (
                    <button key={s} onClick={async () => { await base44.entities.Drop.update(d.id, { status: s }); load(); }} style={{ padding: "6px 12px", fontSize: 8, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit", background: d.status === s ? statusColor(s) : "transparent", color: d.status === s ? "#000" : SD, border: `0.5px solid ${d.status === s ? statusColor(s) : G3}`, fontWeight: d.status === s ? 700 : 400 }}>{s}</button>
                  ))}
