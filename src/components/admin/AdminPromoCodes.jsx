  ...
  return (
    <div>
      ...
      <input
        ...
        style={{ width: "100%", background: G1, border: `.5px solid ${G3}`, color: "#fff", padding: "10px 14px", fontSize: 12, outline: "none", boxSizing: "border-box", fontFamily: "inherit" }}
      />
    </div>
  );
}
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 900, color: "#fff", letterSpacing: -1 }}>Promo Codes</h2>
        </div>
      ...
      {showForm && (
        <div style={{ background: G2, border: `.5px solid ${G3}`, borderTop: `2px solid ${S}`, padding: "24px", marginBottom: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              ...
              style={{ width: 40, height: 22, borderRadius: 11, background: form.active ? S : G3, border: "none", cursor: "pointer", position: "relative", transition: "background .2s" }}
            >
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={save} disabled={saving} style={{ background: S, color: "#000", border: "none", padding: "10px 24px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", fontWeight: 900, cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
              {saving ? "Saving..." : "Save"}
            </button>
            <button onClick={cancel} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "10px 20px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {codes.map(c => {
            ...
            return (
              <div key={c.id} style={{ background: G2, border: `.5px solid ${G3}`, padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  {c.expiresAt && <div style={{ fontSize: 9, color: SD }}>Expires {new Date(c.expiresAt).toLocaleDateString()}</div>}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => toggleActive(c)} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "6px 14px", fontSize: 8, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>
                    {c.active ? "Disable" : "Enable"}
                  </button>
                  <button onClick={() => openEdit(c)} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "6px 14px", fontSize: 8, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>Edit</button>
                  <button onClick={() => del(c.id)} style={{ background: "none", border: `.5px solid #e03`, color: "#e03", padding: "6px 14px", fontSize: 8, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
                </div>
              </div>
            );
          })}
