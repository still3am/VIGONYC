...
function SectionCard({ title, children, accent }) {
  return (
    <div style={{ background: G1, border: `.5px solid ${G3}`, borderTop: accent ? `2px solid ${S}` : `.5px solid ${G3}`, marginBottom: 16 }}>
      <div style={{ padding: "16px 24px", borderBottom: `.5px solid ${G3}`, fontSize: 10, letterSpacing: 3, textTransform: "uppercase", color: "#fff", fontWeight: 700 }}>{title}</div>
...
export default function VigoAdmin() {
    ...
    return (
      <div style={{ background: "#000", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Helvetica Neue',Arial,sans-serif", padding: 24 }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ background: G1, border: `.5px solid ${G3}`, borderTop: `2px solid ${S}`, padding: "40px 36px" }}>
  ...
  return (
    <div style={{ background: "#000", minHeight: "100vh", fontFamily: "'Helvetica Neue',Arial,sans-serif", color: "#fff", display: "flex", flexDirection: "column" }}>
      {/* Admin top bar */}
      <div style={{ background: G1, borderBottom: `.5px solid ${G3}`, padding: "0 24px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => window.open("/", "_blank")} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "7px 16px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>Preview Site</button>
          <button onClick={handleSave} style={{ background: saved ? "#0c6" : S, color: "#000", border: "none", padding: "7px 20px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", fontWeight: 900, cursor: "pointer", fontFamily: "inherit", transition: "background .3s" }}>
            {saved ? "✓ Saved!" : "Save Changes"}
          </button>
          <button onClick={() => setAuthenticated(false)} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "7px 14px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>Logout</button>
        </div>
      </div>

      <div style={{ display: "flex", flex: 1 }}>
        {/* Sidebar nav */}
        <div style={{ width: 220, background: G1, borderRight: `.5px solid ${G3}`, padding: "24px 0", flexShrink: 0 }}>
        ...
        {/* Editor main */}
        <div style={{ flex: 1, padding: "32px 36px", overflowY: "auto", maxHeight: "calc(100vh - 56px)" }}>

          {activeSection === "Hero" && (
            <div>
              <div style={{ fontSize: 9, letterSpacing: 4, color: S, textTransform: "uppercase", marginBottom: 8 }}>✦ Hero Section</div>
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Homepage Hero</h2>
          ...
          {activeSection === "Banner" && (
            <div>
              ...
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Ticker & Drop Banner</h2>
          ...
          {activeSection === "Products" && (
            <div>
              ...
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Product Manager</h2>
              {settings.products.map(p => (
                <SectionCard key={p.id} title={`Product ${p.id}`} accent={p.visible}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 16, alignItems: "end" }}>
                    ...
                    <div>
                      ...
                      <button onClick={() => updateProduct(p.id, "visible", !p.visible)} style={{ background: p.visible ? "#0c6" : G2, color: p.visible ? "#000" : SD, border: `.5px solid ${p.visible ? "#0c6" : G3}`, padding: "10px 16px", fontSize: 9, cursor: "pointer", fontFamily: "inherit", letterSpacing: 1 }}>
                        {p.visible ? "ON" : "OFF"}
                      </button>
                    </div>
                  </div>
          ...
          {activeSection === "Brand Story" && (
            <div>
              ...
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Brand Story</h2>
          ...
          {activeSection === "Contact & Social" && (
            <div>
              ...
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Contact & Social</h2>
          ...
          {activeSection === "Theme" && (
            <div>
              ...
              <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1, marginBottom: 28 }}>Theme & Colors</h2>
          ...
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: `.5px solid ${G3}`, display: "flex", gap: 12 }}>
            <button onClick={handleSave} style={{ background: saved ? "#0c6" : S, color: "#000", border: "none", padding: "14px 36px", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", fontWeight: 900, cursor: "pointer", fontFamily: "inherit", transition: "background .3s" }}>
              {saved ? "✓ Saved!" : "Save Changes"}
            </button>
            <button onClick={() => window.open("/", "_blank")} style={{ background: "none", border: `.5px solid ${G3}`, color: SD, padding: "14px 28px", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "inherit" }}>
              Preview Site →
            </button>
          </div>
        </div>
      ...
      <style>{`
        @media(max-width:768px){
          ...
          .admin-sidebar{width:100% !important; flex-direction:row !important; overflow-x:auto;}
        }
      `}</style>
    </div>
  );
